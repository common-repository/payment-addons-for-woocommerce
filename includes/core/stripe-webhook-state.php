...
/**
 * Class wspa_Stripe_Webhook_State.
 *
 * Tracks the most recent successful and unsuccessful webhooks in test and live modes.
 *
 * @since 5.0.0
 */
class Stripe_Webhook_State {
	const OPTION_LIVE_MONITORING_BEGAN_AT = 'wspa_stripe_wh_monitor_began_at';
	const OPTION_LIVE_LAST_SUCCESS_AT     = 'wspa_stripe_wh_last_success_at';
	const OPTION_LIVE_LAST_FAILURE_AT     = 'wspa_stripe_wh_last_failure_at';
	const OPTION_LIVE_LAST_ERROR          = 'wspa_stripe_wh_last_error';

	const OPTION_TEST_MONITORING_BEGAN_AT = 'wspa_stripe_wh_test_monitor_began_at';
	const OPTION_TEST_LAST_SUCCESS_AT     = 'wspa_stripe_wh_test_last_success_at';
	const OPTION_TEST_LAST_FAILURE_AT     = 'wspa_stripe_wh_test_last_failure_at';
	const OPTION_TEST_LAST_ERROR          = 'wspa_stripe_wh_test_last_error';
    ...
    return Stripe_Settings::is_test_mode();
	}
	...
	public static function get_last_error_reason() {
		...
		if ( self::VALIDATION_SUCCEEDED == $last_error ) {
			return( __( 'No error', 'elementor-pay-addons' ) );
		}

		if ( self::VALIDATION_FAILED_EMPTY_HEADERS == $last_error ) {
			return( __( 'The webhook was missing expected headers', 'elementor-pay-addons' ) );
		}

		if ( self::VALIDATION_FAILED_EMPTY_BODY == $last_error ) {
			return( __( 'The webhook was missing expected body', 'elementor-pay-addons' ) );
		}

		if ( self::VALIDATION_FAILED_USER_AGENT_INVALID == $last_error ) {
			return( __( 'The webhook received did not come from Stripe', 'elementor-pay-addons' ) );
		}

		if ( self::VALIDATION_FAILED_SIGNATURE_INVALID == $last_error ) {
			return( __( 'The webhook signature was missing or was incorrectly formatted', 'elementor-pay-addons' ) );
		}

		if ( self::VALIDATION_FAILED_TIMESTAMP_MISMATCH == $last_error ) {
			return( __( 'The timestamp in the webhook differed more than five minutes from the site time', 'elementor-pay-addons' ) );
		}

		if ( self::VALIDATION_FAILED_SIGNATURE_MISMATCH == $last_error ) {
			return( __( 'The webhook was not signed with the expected signing secret', 'elementor-pay-addons' ) );
		}

		return( __( 'Unknown error.', 'elementor-pay-addons' ) );
	}
	...
	public static function get_webhook_status_message() {
		...
		if ( $last_success_at > $last_failure_at ) {
			$message = sprintf(
				$test_mode ?
					...
					__( 'The most recent test webhook, timestamped %s, was processed successfully.', 'elementor-pay-addons' ) :
					/* translators: 1) date and time of last webhook received, e.g. 2020-06-28 10:30:50 UTC */
					__( 'The most recent live webhook, timestamped %s, was processed successfully.', 'elementor-pay-addons' ),
		...
		if ( ( 0 == $last_success_at ) && ( 0 == $last_failure_at ) ) {
			$message = sprintf(
				$test_mode ?
					...
					__( 'No test webhooks have been received since monitoring began at %s.', 'elementor-pay-addons' ) :
					/* translators: 1) date and time webhook monitoring began, e.g. 2020-06-28 10:30:50 UTC */
					__( 'No live webhooks have been received since monitoring began at %s.', 'elementor-pay-addons' ),
		...
		if ( $last_success_at > 0 ) {
			$message = sprintf(
				$test_mode ?
					 ...
					 */
					__( 'Warning: The most recent test webhook, received at %1$s, could not be processed. Reason: %2$s. (The last test webhook to process successfully was timestamped %3$s.)', 'elementor-pay-addons' ) :
					/*
					 * translators: 1) date and time of last failed webhook e.g. 2020-06-28 10:30:50 UTC
					 * translators: 2) reason webhook failed
					 * translators: 3) date and time of last successful webhook e.g. 2020-05-28 10:30:50 UTC
					 */
					__( 'Warning: The most recent live webhook, received at %1$s, could not be processed. Reason: %2$s. (The last live webhook to process successfully was timestamped %3$s.)', 'elementor-pay-addons' ),
		...
		$message = sprintf(
			$test_mode ?
				 ...
				 */
				__( 'Warning: The most recent test webhook, received at %1$s, could not be processed. Reason: %2$s. (No test webhooks have been processed successfully since monitoring began at %3$s.)', 'elementor-pay-addons' ) :
				/* translators: 1) date and time of last failed webhook e.g. 2020-06-28 10:30:50 UTC
				 * translators: 2) reason webhook failed
				 * translators: 3) date and time webhook monitoring began e.g. 2020-05-28 10:30:50 UTC
				 */
				__( 'Warning: The most recent live webhook, received at %1$s, could not be processed. Reason: %2$s. (No live webhooks have been processed successfully since monitoring began at %3$s.)', 'elementor-pay-addons' ),
			gmdate( $date_format, $last_failure_at ),
			$last_error,
			gmdate( $date_format, $monitoring_began_at )
		);
	...
	public static function is_no_event_received() {
		$last_success_at     = self::get_last_webhook_success_at();
		$last_failure_at     = self::get_last_webhook_failure_at();	
		return ( 0 == $last_success_at ) && ( 0 == $last_failure_at );
	}
};
